/**
 * note.com 記事ページの DOM を Markdown 本文に変換する。
 */
(function (global) {
    ...
    while (guard++ < 10000) {
      ...
      if (strongLeaves.length > 0) {
        strongLeaves.forEach((el) => {
          const innerText = (el.textContent ?? "").replace(/\*/g, "\\*");
          replaceElementWithText(el, `**${innerText}**`);
        });
        continue;
      }
      break;
  ...
  const convertNotePageToMarkdown = (
    ...
    const body = articleElementToMarkdown(articleRoot, pageUrl);
