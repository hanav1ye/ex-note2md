...
(function (global) {
  ...
  const extensionFromMime = (mime) => {
    const normalized = String(mime ?? "")
      ...
      .trim()
      .toLowerCase();
    return MIME_TO_IMAGE_EXTENSION[normalized] ?? "";
  };
  ...
  /**
   * 画像URLを取得し Blob と MIME を返す。
   * @param {string} url - 画像URL。
   * @returns {Promise<{blob: Blob, mime: string}>} 取得結果。
   */
  ...
  const processMarkdownImages = async (markdown, options = {}) => {
    ...
    while ((match = pattern.exec(markdown)) !== null) {
      ...
      try {
        if (mode === "base64") {
          const { blob, mime } = await fetchImageBlob(url);
          const ext = extensionFromMime(mime) || extensionFromUrl(url) || "png";
          imageIndex += 1;
