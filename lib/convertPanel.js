// 変換パネル: 変換元・出力先の選択と変換実行。popup とサイドパネルの両方から読み込まれる。
...
/**
 * サイドパネルとして動いているか。sidepanel.html は <html data-context="sidepanel"> を持つ。
 * popup と違ってフォーカスを失っても閉じず、タブを移動しても残るため、
 * 「自分を閉じる」「開いたときの状態を前提にする」処理はここで切り替える。
 */
const isSidePanel = document.documentElement.dataset.context === "sidepanel";
...
/*
 * popup からサイドパネルを開く。
 * sidePanel.open() はユーザー操作の直後でないと拒否されるため、await を挟まずに
 * WINDOW_ID_CURRENT で現在のウィンドウを指定する。開けたら popup は役目を終えるので閉じる。
 */
openSidePanelBtn?.addEventListener("click", () => {
  ...
  chrome.sidePanel
    ...
    .then(() => window.close())
    .catch(() => {
      setStatus(t("popup.error.openSidePanelFailed"), "error");
    });
});

/*
 * サイドパネルは開いたまま別のタブへ移れるので、アクティブなタブが変わったら
 * 「処理対象」の記事タイトルを取り直す。popup は開くたびに読み直すので不要。
 */
if (isSidePanel) {
